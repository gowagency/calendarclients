...
export default function PostSheet({ post, onClose }: PostSheetProps) {
  ...
  const handleSave = useCallback(() => {
    ...
    const { id, createdAt, updatedAt, sortOrder, checklist, pilar, ...rest } = editingPost as any;
    updatePost.mutate({
      id,
      ...rest,
              ...
              <section style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                ...
                <div>
                  ...
                  {(() => {
                    ...
                    return (
                      <select
                        value={(editingPost as any).pilar || ''}
                        onChange={e => (updateField as any)('pilar', e.target.value || null)}
                        style={{
                          width: '100%', fontSize: '0.85rem', fontWeight: activePilar ? 600 : 400,
                          color: activePilar ? activePilar.color : 'var(--text-secondary)',
                          background: activePilar ? `${activePilar.color}10` : 'var(--bg-elevated)',
                          border: activePilar ? `1px solid ${activePilar.color}40` : '1px solid var(--border)',
                          borderRadius: '8px', padding: '0.5rem 0.75rem', cursor: 'pointer',
